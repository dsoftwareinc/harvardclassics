...
- **TypeScript is pinned to the 6.0 line.** Angular 22's `@angular/compiler-cli` declares `typescript: ">=6.0 <6.1"`. Installing TypeScript 7 makes every build fail at startup with a non-obvious `TypeError: Cannot read properties of undefined (reading 'Error')` from `readConfiguration`. When `ncu -u` bumps `typescript` past 6.0.x, put it back.
- **Ionic 9 moved the NgModule build to a subpath.** In Ionic 8, `@ionic/angular` resolved to the lazy/NgModule build; in Ionic 9 the bare specifier resolves to the **standalone** build, which has no `IonicModule`. This app is NgModule-based, so all TS imports use `@ionic/angular/lazy` (`IonicModule`, `IonicRouteStrategy`, `ActionSheetController`, `ScrollCustomEvent`, …). Importing from `@ionic/angular` instead compiles the app but fails at template-check time with a wall of `NG8001: 'ion-*' is not a known element`. The `@ionic/angular/css/*` imports in `global.scss` are unaffected.
